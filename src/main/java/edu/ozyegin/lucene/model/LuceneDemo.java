...
//        luceneDemo.deletDocumentFromIndex();
    ...
    private void createIndexSearcher() throws CorruptIndexException, IOException{
		 ...
		 */
        DirectoryReader var18 = DirectoryReader.open(FSDirectory.open(Paths.get(indexDirectory, new String[0])));
        indexSearcher = new IndexSearcher(var18);
    }
    ...
    private void showSearchResults(Query query ){
        ...
        try{
            ...
            for(ScoreDoc scoredoc: scoreDosArray){
                ...
                System.out.println("\nSender: " + doc.getField("baslik").stringValue());
                System.out.println("Subject: " + doc.getField("yil").stringValue());
                System.out.println("Email file location: " + doc.getField("yazarlar").stringValue());
            }
            System.out.println("---------------------------------------------");
        }catch(IOException e){
            e.printStackTrace();
    ...
    private void termQueryExample(){
        ...
        Term term = new Term("baslik", "YOLONSEL");
